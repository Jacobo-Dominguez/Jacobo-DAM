...
class FrameReservas(ttk.Frame):
    ...
    def _cargar_reservas(self):
        ...
        for r in self.ctrl.listar():
            ...
            aparato = self.ctrl_aparatos.obtener_por_id(r.id_aparato)
            nombre_cliente = f"{cliente.nombre} {cliente.apellido}" if cliente else "Desconocido"
            nombre_aparato = aparato.nombre if aparato else "Desconocido"
            self.tabla.insert("", "end", values=(r.id, r.id_cliente, nombre_cliente, nombre_aparato, r.fecha, r.hora))
    ...
    def _filtrar(self):
        ...
        for r in self.ctrl.listar(filtro):
            self.tabla.insert("", "end", values=(r.id, r.id_cliente, r.id_aparato, r.fecha, r.hora))
    ...
    def _abrir_formulario_agregar(self):
        FormularioReserva(self, "Agregar Reserva", self._guardar_nueva_reserva)

    def _abrir_formulario_editar(self):
        id_reserva = self._get_reserva_seleccionada()
        if not id_reserva:
            return
        reserva = self.ctrl.obtener_por_id(id_reserva)
        FormularioReserva(self, "Editar Reserva", lambda datos: self._guardar_edicion_reserva(id_reserva, datos), reserva)
